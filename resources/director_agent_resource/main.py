...
from .tools import call_scene_planner_agent
...
tools = [call_scene_planner_agent]
...
async def tool_node(state: dict):
    ...
    log_message(f"Director agent executing tools in parallel: {tool_names}")
    ...
    async def call_tool_individually(tool_call):
        ...
        log_important_step(
            f"Director agent executing tool: {tool_name}",
        ...
        if tool_name in tools_by_name:
            ...
            log_error(
                "Director agent received unknown tool request",
...
async def compile_director_agent_graph(
    thread_id=None,
    human_message="Hey!",
):
    ...
    log_message(f"Director agent finished | Thread id: {thread_id}")
